...
class Using_goods extends Parent_controller {

  var $parsing_form_input = array('id','kode_produksi','id_barang','qty','satuan','tgl_produksi','user_insert','date_insert');
  var $tablename = 't_using_goods';

    public function __construct() {
        parent::__construct();
        $this->load->model('m_using_goods','m_usg');
    ...
    public function index() {
        ...
        $data['listing'] = $this->m_usg->list_using_goods();
        $data['parse_view'] = 'using_goods/using_goods_view';
    ...
    public function store(){
        ...
        if($id == '' || empty($id) || $id == NULL){
          $data['parseform'] = $this->m_usg->get_new($this->parsing_form_input);
        }else{
          $data['parseform'] = $this->m_usg->get_all($id,$this->tablename)->row();

        }
        $data['opt_barang'] = $this->m_usg->opt_goods();
        $data['kode_produksi'] = $this->transaksi_id();
        $data['parse_view'] = 'using_goods/using_goods_store';
    ...
    public function get_content_form(){
      $id = $this->uri->segment(3);
      $sql = $this->db->query("select * from m_goods where id = '$id' ")->row();
    ...
    public function save(){
      ...
      $datapos = $this->m_usg->input_array($this->parsing_form_input);
      $id = isset($datapos['id']) ? $datapos['id'] : '';
      $save = $this->m_usg->save($datapos,$id,$this->tablename);


      $save_update = $this->m_usg->update_when_save($datapos);
      if($save && $save_update){
        echo "<script language=javascript>
         alert('Simpan Data Berhasil');
         window.location='" . base_url('using_goods') . "';
             </script>";
      }
    ...
    public function delete(){
      $datapos = $this->uri->segment(3);

      $sql = $this->db->where("id",$datapos)->get("t_using_goods")->row();
      $save_delete= $this->m_usg->update_when_delete($sql->id_barang,$sql->qty);
      
      $del = $this->m_usg->delete($datapos,$this->tablename);
      //var_dump($sql);
      //exit();
    ...
    public function get_bawahan_foreman($idforeman) {
        ...
        $data['idusing_goods'] = $this->session->userdata('idusing_goods');
    ...
    public function add() {
        ...
        $data['opt_using_goods'] = $this->model_user_management->opt_using_goods();
    ...
    public function get_val_peg() {
        ...
        $data = $this->db->get('tb_using_goods')->row();
    ...
    public function pro_add() {
        ...
        $datapos = array('nrp' => $this->input->post('nrp'),
            ...
            'id_using_goods' => $this->input->post('id_using_goods'),
