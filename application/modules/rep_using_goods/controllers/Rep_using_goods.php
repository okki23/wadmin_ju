...
class Rep_using_goods extends Parent_controller {
  ...
  var $tablename = 't_using_goods';
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('m_rep_using_goods','mrug');
    ...
    public function index() {
        ...
        $data['parse_view'] = 'rep_using_goods/rep_using_goods_view';
        $data['listing'] = $this->mrug->list_rep_using_goods();
    ...
    public function process(){
          ...
          $end = date('Y-m-d', strtotime($this->input->post('end')));
              $data['listing'] = $this->mrug->print_rep_using_goods($start,$end);
              $data['start'] = $start;
              $data['end'] = $end;
              ...
              $html = $this->load->view('rep_using_goods/rep_using_goods_print', $data, true);
    ...
    $data['judul'] = $this->data['judul'];
     $id = $this->uri->segment(3);
          ...
          $data['parse_view'] = 'rep_using_goods/rep_using_goods_store';
          //session
          $data['username'] = $this->session->userdata('username');
          $data['user_group'] = strtoupper(level_help($this->session->userdata('user_group')));
          $data['user_id'] = $this->session->userdata('user_id');
    ...
    public function save(){
      ...
      if($id == '' || $id == NULL){
            ...
            foreach($sql as $listrec){
              ...
              $this->email->message('Information to '.$listrec->nama.' The New PO has been released ,please paid this PO to the supplier <br> This link will be forwarding you to the apps : ' .base_url('rep_using_goods'). '<br> Thanks');
              $this->email->send();
              $this->email->clear();
        ...
        foreach($sql as $listrec){
          ...
          $this->email->message('Information to '.$listrec->nama.' PO has been updated ,please check or paid this PO to the supplier <br> This link will be forwarding you to the apps : ' .base_url('rep_using_goods'). '<br> Thanks');
          $this->email->send();
          $this->email->clear();
      ...
      if($save && $id == NULL || $id == ''){
              echo "<script language=javascript>
               ...
               window.location='" . base_url('rep_using_goods') . "';
                   </script>";
        }
